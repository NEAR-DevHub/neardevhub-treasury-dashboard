...
async function checkBalance(
  ...
  try {
    ...
    if (result.result && result.result.length > 0) {
      ...
      console.log(
        `💰 ${tokenSymbol} Balance: ${formattedBalance} ${tokenSymbol}`
      );
      return balance;
    } else {
      console.log(
        `💰 ${tokenSymbol} Balance: 0 ${tokenSymbol} (not registered or no balance)`
      );
      return "0";
    }
  } catch (error) {
...
async function get1ClickQuote(usdcAmount) {
  ...
  console.log(
    `Amount: ${(parseInt(usdcAmount) / 1000000).toFixed(
      6
    )} USDC (NEAR) → USDC (Ethereum)`
  );
...
Monitor status: https://explorer.near-intents.org/?depositAddress=${
    quote.response.quote.depositAddress
  }
...
async function main() {
  ...
  if (intentsTokens && intentsTokens.length > 0) {
    usdcInIntents = intentsTokens.find(
      (token) => token.token_id === `nep141:${USDC_CONTRACT}`
    );
    if (usdcInIntents) {
      availableUsdcAmount = Math.max(
        availableUsdcAmount,
        parseInt(usdcInIntents.balance)
      );
      console.log(
        `💡 Found USDC in NEAR Intents: ${(
          parseInt(usdcInIntents.balance) / 1000000
        ).toFixed(6)} USDC`
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `   Signature: ${quote.response.signature.substring(0, 50)}...`
    );
    ...
    console.log(`\n🎯 SUBMISSION COMMAND:`);
    console.log(
      `near contract call-function as-transaction ${DAO_ACCOUNT} add_proposal file-args dao-proposal.json prepaid-gas '100.0 Tgas' attached-deposit '0.1 NEAR' sign-as <your-account.near> network-config mainnet sign-with-keychain send`
    );

    console.log(`\n📝 MANUAL SUBMISSION STEPS:`);
    console.log(`1. 📁 The proposal has been saved to dao-proposal.json`);
    console.log(
      `2. 🔑 Replace <your-account.near> with your DAO member account`
    );
    console.log(`3. 📤 Execute the command above to submit the proposal`);
    console.log(`4. 🗳️  Wait for DAO members to vote and approve`);
    console.log(
      `5. ⚡ Execute the approved proposal to transfer USDC to deposit address`
    );
    console.log(
      `6. 🔍 Monitor swap progress: https://explorer.near-intents.org/?depositAddress=${quote.response.quote.depositAddress}`
    );
    console.log(
      `7. 🎉 Verify USDC (Ethereum) delivery to DAO's NEAR Intents account`
    );
    ...
    console.log(
      `- Deposit address verified: ${quote.response.quote.depositAddress}`
    );
