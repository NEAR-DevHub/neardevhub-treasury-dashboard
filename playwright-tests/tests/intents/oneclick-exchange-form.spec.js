...
test.describe("OneClickExchangeForm Component", () => {
  ...
  // Helper function to mock RPC responses
  const mockRpcResponses = async (page, theme = "light") => {
    await page.route("**/rpc", async (route) => {
      ...
      } else if (body.params?.method_name === "mt_batch_balance_of" || 
  ...
  test("renders in light theme", async ({ page, instanceAccount }) => {
    ...
    await mockRpcResponses(page, "light");
    await setupComponent(page, instanceAccount, instanceAccount, "light");
    ...
    await expect(page.locator(".balance-header")).toContainText("Available Balance");

    // Check form sections
    await expect(page.locator('.form-section:has(.form-label:text("Send"))')).toBeVisible();
    await expect(page.locator('.form-section:has(.form-label:text("Receive"))')).toBeVisible();
    await expect(page.locator('.form-section:has(.form-label:text("Network"))')).toBeVisible();
    await expect(page.locator('.form-section:has(.form-label:text("Price Slippage Limit"))')).toBeVisible();
    ...
    const containerStyles = await page.locator(".one-click-exchange-form").evaluate((el) => {
  ...
  test("renders in dark theme", async ({ page, instanceAccount }) => {
    await mockApiResponses(page);
    await mockRpcResponses(page, "dark");
    await setupComponent(page, instanceAccount, {}, "dark");

    // Check that component renders
    await expect(page.locator(".one-click-exchange-form")).toBeVisible();
    
    // Verify dark theme is applied by checking for dark theme CSS custom properties
    const hasThemeVariables = await page.evaluate(() => {
      const style = document.documentElement.style;
      // Check if CSS custom properties are set (they would be injected by getAllColorsAsObject)
      return style.length > 0 || 
             document.querySelector("style[data-theme]") !== null ||
             getComputedStyle(document.documentElement).getPropertyValue("--bg-page-color");
    });
  ...
  test("handles slippage tolerance input", async ({ page, instanceAccount }) => {
    await mockApiResponses(page);
    await mockRpcResponses(page);
    await setupComponent(page, instanceAccount);

    // Find slippage input
    const slippageInput = page.locator('.form-section:has-text("Price Slippage Limit") input');
    ...
    await slippageInput.fill("0.1");
    await expect(slippageInput).toHaveValue("0.1");

    await slippageInput.fill("50");
    await expect(slippageInput).toHaveValue("50");

    // Check that the percentage symbol is displayed
    await expect(page.locator('.form-section:has-text("Price Slippage Limit") span:text("%")')).toBeVisible();
  });

  test("displays quote and calculates minimum received", async ({ page, instanceAccount }) => {
    await mockApiResponses(page);
    await mockRpcResponses(page);
    await setupComponent(page, instanceAccount);
    ...
    await page.evaluate(() => {
      const button = document.querySelector('button:has-text("Get Quote")');
      if (button) {
    ...
    // Check quote display appears
    const quoteDisplay = page.locator(".quote-display");
    if (await quoteDisplay.isVisible()) {
      // Check expiry alert
      await expect(page.locator(".quote-alert")).toBeVisible();
      await expect(page.locator(".quote-alert")).toContainText("Please approve this request within");
      ...
      const minReceivedRow = page.locator('.detail-row:has(.detail-label:text("Minimum received"))');
  ...
  test("displays loading states", async ({ page, instanceAccount }) => {
    ...
    // Check initial loading state for balances
    await expect(page.locator(".balance-list")).toContainText("Loading balances...");
  ...
  test("handles quote expiry time calculation", async ({ page, instanceAccount }) => {
    await mockApiResponses(page);
    await mockRpcResponses(page);
    await setupComponent(page, instanceAccount);
    ...
    for (const testCase of testExpiryTimes) {
      ...
      if (testCase.expected === "expired") {
        ...
        // For time-based results, just check that it contains the unit (minutes, hours, days)
        const unit = testCase.expected.split(" ")[1];
        expect(result).toMatch(new RegExp(unit));
      }
    }
  });
  ...
  test("validates quote deadline", async ({ page, instanceAccount }) => {
    ...
    await page.evaluate(() => {
      ...
      errorDiv.innerHTML = '<i class="bi bi-exclamation-triangle-fill me-2"></i>Invalid quote: No expiry deadline provided';
      document.querySelector(".one-click-exchange-form").appendChild(errorDiv);
    });

    await expect(page.locator(".alert-danger")).toContainText("Invalid quote: No expiry deadline provided");
  });
});