...
test("Web3IconFetcher BOS compatibility test loads and displays icons", async ({
  ...
  await expect(page.locator("p").filter({ hasText: "Testing single Web3IconFetcher instance" })).toBeVisible();
  ...
  await expect(page.locator("h4").filter({ hasText: "Results:" })).toBeVisible();
  ...
  if (cacheKeys.length === 0) {
    console.warn("No icon cache entries found - this may indicate a loading issue");
  ...
  await expect(page.locator("h4").filter({ hasText: "Icon Display Test:" })).toBeVisible();
  ...
  if (iconCount > 0) {
    ...
    console.log("No token icons found - this might indicate an issue with icon loading");
  }
