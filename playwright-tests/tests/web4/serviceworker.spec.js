...
test.describe("Web4 Service Worker", () => {
  ...
  test("should cache RPC requests and serve them on reload", async ({
    browser,
  }, testInfo) => {
    // Skip this test if not running on the treasury-testing project
    test.skip(
    ...
    try {
      ...
      if (hasSwLogs) {
        ...
        const freshFetchLogs = swLogs.filter((log) =>
          log.includes("Fetching fresh RPC response")
        );
        console.log(
          `📊 First load - Fresh fetch logs: ${freshFetchLogs.length}`
        );
        ...
        const cachedResponseLogs = swLogs.filter((log) =>
          log.includes("Returning cached RPC response")
        );
        const freshFetchLogsAfterReload = swLogs.filter((log) =>
          log.includes("Fetching fresh RPC response")
        );

        console.log(
          `📊 After reload - Cached response logs: ${cachedResponseLogs.length}`
        );
        console.log(
          `📊 After reload - Fresh fetch logs: ${freshFetchLogsAfterReload.length}`
        );

        // We should have some cached responses on reload (less fresh fetches than first load)
        if (cachedResponseLogs.length > 0) {
          console.log(
            `✅ Cache is working: Found ${cachedResponseLogs.length} cached responses`
          );
        } else if (freshFetchLogsAfterReload.length < freshFetchLogs.length) {
          console.log(
            `✅ Cache may be working: Fewer fresh fetches on reload (${freshFetchLogsAfterReload.length} vs ${freshFetchLogs.length})`
          );
        } else {
          console.log(
            `⚠️  Cache might not be working optimally: Same number of fresh fetches`
          );
        }
      } else {
        console.log(
          `⚠️  No service worker logs detected - service worker may not be intercepting requests`
        );
      }
  ...
  test("should verify service worker cache storage directly", async ({
    browser,
  }, testInfo) => {
    // Skip this test if not running on the treasury-testing project
    test.skip(
    ...
    try {
      ...
      console.log(
        `🌐 Navigating to ${testServerInfo.url}/?page=payments&tab=history`
      );
      ...
      if (treasuryCacheExists) {
        ...
        if (cacheSize > 0) {
          console.log(
            `✅ Service worker cache is working! Found ${cacheSize} cached items`
          );
        } else {
          console.log(
            `ℹ️ Cache exists but is empty - may need more time or different requests`
          );
        }
        ...
        console.log(
          `ℹ️ Treasury cache not found - service worker may not have cached anything yet`
        );
        console.log(
          `ℹ️ This could be due to timing, request patterns, or cache policies`
        );
      }
