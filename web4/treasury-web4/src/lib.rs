...
impl Contract {
    pub fn update_widgets(&mut self) {        
        socialdb::ext(SOCIALDB_ACCOUNT_ID.parse().unwrap()).get(
            [
                format!("{}/widget/**", WIDGET_REFERENCE_ACCOUNT_ID)
            ].to_vec()
    ...
    pub fn update_widgets_callback(&mut self) {
        ...
        match env::promise_result(0) {
            PromiseResult::Successful(result) => {
                let mut widget: Value = near_sdk::serde_json::from_slice(result.as_slice()).unwrap();
                widget[env::current_account_id().to_string()] = widget[WIDGET_REFERENCE_ACCOUNT_ID].clone();
