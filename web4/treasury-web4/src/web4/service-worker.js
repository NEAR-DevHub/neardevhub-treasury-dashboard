...
const BUILD_TIMESTAMP = 1757671421460;
...
async function handleRpcRequest(request) {
  try {
    ...
    if (jsonBody && jsonBody.params) {
      ...
      if (
        accountId &&
        accountId.endsWith(".sputnik-dao.near") &&
        methodName &&
        methodName.includes("proposal")
      ) {
        swLog(
    ...
    if (cachedResponse) {
      ...
      if (cacheTime && Date.now() - parseInt(cacheTime) < duration) {
        ...
        swLog(
          `Service Worker: Cache expired, removing entry if it is not 'invalidateAll'`
        );
        // Cache expired, remove it
        if (
          specialCacheDuration === null ||
          !specialCacheDuration.invalidateAll
        ) {
        ...
        if (response.status === 200) {
          ...
          if (specialCacheDuration && specialCacheDuration.invalidateAll) {
            try {
              ...
              const prevCachedResponse = await prevCache.match(
                prevCacheRequest
              );
              if (prevCachedResponse) {
                ...
                if (JSON.stringify(prevArr) !== JSON.stringify(newArr)) {
                  swLog(
                    "Service Worker: Proposal result changed, invalidating ALL caches."
                  );
              ...
              swLog(
                "Service Worker: Error comparing previous and new proposal response: " +
                  e.message
              );
            }
          }
