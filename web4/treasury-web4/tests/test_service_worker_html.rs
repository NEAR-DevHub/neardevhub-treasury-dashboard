...
async fn test_service_worker_html_contains_registration() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let has_registration =
        body_string.contains("navigator.serviceWorker.register('/service-worker.js')");
    ...
    assert!(
        has_service_worker,
        "HTML should contain service worker registration"
    );
    assert!(
        has_registration,
        "HTML should contain service worker registration call"
    );
